...
for (i in cuts) {
  for (j in pwrs) {
    cat(sprintf("Performing clustering for mergeCutHeight %0.2f and power %d\n",i,j))
	  ...
	  write.table(t(net$MEs),file=paste(TAG,"_power_",j,"_mch_",i,"_eigengenes.txt",sep=""),quote=F,col.names=NA,row.names=T,sep="\t")
  }
}
...
wgcna_preprocessed_parasens2 = function(csvfile,ngenes=6000) {
  ## v 3.0 parameter sensitivity testing
  ## cut 0.05, min cluster size 100   
  library (WGCNA)
  stringsAsFactors=F
  enableWGCNAThreads()
	cat(sprintf("Processing file %s, top %d most expressed genes will be considered\n",csvfile,ngenes))
  ...
  for (j in pwrs) {
    cat(sprintf("Performing clustering for mergeCutHeight 0.05 and power %d\n",j))
	  net <- blockwiseModules(texp, power = j, minModuleSize = 100, reassignThreshold = 0, mergeCutHeight = 0.05,
	                          maxBlockSize=10000,deepSplit=4,numericLabels=T,pamRespectsDendro=F,networkType = "signed hybrid",verbose = 0)
	  print(table(net$colors))
	  write.table(cbind(rownames(exp),net$colors),file=paste(TAG,"_power_",j,"_hybrid_modules.txt",sep=""),quote=F,col.names=F,row.names=F,sep="\t")
	  write.table(t(net$MEs),file=paste(TAG,"_power_",j,"_eigengenes.txt",sep=""),quote=F,col.names=colnames(exp),row.names=T,sep="\t")
  }
	
	cat(sprintf("CLUSTERING IS COMPLETE. See files *modules.txt for the list of modules and  *eigengenes.txt for averaged module expression.\n"))
}

wgcna_preprocessed_prod1 = function(csvfile,ngenes=6000) {
...
run_deseq2_hm = function (cond1,cond2,condfile,expmatrix) {
  ...
  ## version for expression matrix format.        
    library(DESeq2)
    condTable <- read.table(condfile,header=F,col.names=c("sampleID","condition","donor"),stringsAsFactors=F)

    exp <- read.table(expmatrix,header=T,row.names=1,check.names=F)
    cond12 <- condTable[condTable$condition==cond1 | condTable$condition==cond2,]
   ## this is neat
    exp12 <- exp[colnames(exp)%in%cond12$sampleID]
    ## for ESAT and rsem output
    exp12r <- round(exp12,0)
    print("Processing the following samples and conditions:")
    print(cond12)
    print("--------------------------------------------------")
    ddsMatrix<-DESeqDataSetFromMatrix(countData=as.matrix(exp12r),colData=cond12,design = ~ donor + condition)

    ddsMatrix <- DESeq(ddsMatrix)
    res <- results(ddsMatrix)
    annfile <- "/home/apredeus/useful/Gencode_v19.ensembl_gene_ID_to_symbol.list"
    ann <- read.table(annfile,header=F,row.names=1,col.names=c("Entrez_gene_ID","Gene_symbol"))
    resAnn <- cbind(ann,res)
    resOrdered <- resAnn[order(resAnn$padj),]
    tag <- paste(cond1,"vs",cond2,sep="_")
    output <- paste(tag,".DESeq.out",sep="")
    write.table(resOrdered,file=output,quote=F)
}
